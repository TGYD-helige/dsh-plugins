...
**Scope boundary:** Implement reusable dsh or protocol capabilities that make sense across consuming applications. Keep one application's identity headers, database fields, and business workflows in that application. Before adding a plugin option or compatibility path for an application request, establish a concrete use case independent of that application.

- `dsh-a2a` — A2A protocol (JSON-RPC + SSE) server driving `ctx.agents`; inbound file parts (inline bytes / `url`, plugin-side bounded download) map onto the optional `attachments` service (`dsh-attachment` image/file content blocks), else persist under the configured `uploadsDir` (default `<OS temp>/dsh-a2a-uploads/<date>/`) and ride a `<document>` path reference the agent opens with its own tools; `data` parts become `<data>` JSON text — a turn never fails over part kinds
...
- `dsh-policy` — declarative tool-call policy on the `tools/pre-execute` gate: config-driven `allow`/`deny`/`ask` rules (tool name(s), any-of `argsPattern` on JSON-stringified args, per-segment any-of `commandPrefix`/segment-start-anchored `commandRegex` on shell commands, priority with fail-closed tie-break; deny-anywhere across compound segments, allow only when every segment decided allow; no match → `next()`). `ask` defers to dsh's own approval seam (`ctx.approval`, fail-closed); invalid regexes fail the plugin load. Shell segmentation (`src/shell.ts`) is quote-aware and extracts `$( )`/backtick interiors as own segments.
...
- **Peer deps**: `@deepseek-ai/*` packages and heavy clients (`@langfuse/tracing` + `@langfuse/otel`, `@prisma/client`, `ioredis`, `@google-cloud/storage`) go in `peerDependencies` — never bundle the harness or optional backends. dsh peers are fixed minimum-version floors — never pins, carets, or per-release bumps: the floor moves only when a plugin starts requiring a newer dsh API. `dsh plugin add` is pnpm-only and pnpm's peer matching has no prerelease gate (verified: `>=0.1.5-rc.1` accepts `0.1.6-alpha.2` without warnings), so newer dsh versions never block installs there. npm DOES gate prereleases per release tuple, so while a dsh line is pre-release the floor carries a union member naming its alpha tuple: `>=0.1.5-rc.1 || >=0.1.6-alpha.1`. Drop the union member once that line ships a final release (finals satisfy the plain floor). Uniform rule across packages: peerDeps = exactly the dsh packages whose services/events/types the plugin consumes; devDependencies pin the tested dsh version and cover every `@deepseek-ai/*` package src or tests import (type-only event-augmentation imports count).
