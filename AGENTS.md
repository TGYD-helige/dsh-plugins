...
PR automation (GitHub-hosted, `pull_request_target`; PR content flows in as untrusted data, only trusted base code is checked out): `.github/workflows/labeler.yml` syncs rule labels (`area/*`, `type/*`, `size:*`, `platform/*`) on every PR and proposes P0–P3 for same-repo PRs via the integration gateway; `.github/workflows/dsh-review.yml` posts a three-axis (Standards/Spec/Ponytail) DSH review on non-draft PRs — P0/P1 findings fail the job (Ponytail is taste-level and never blocks). `pull_request_target` workflows only trigger once the workflow file exists on the default branch — they start working after the PR that adds them merges.
